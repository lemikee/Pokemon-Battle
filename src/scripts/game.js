...
export default class Game {
  ...
  start() {
    ...
    document.getElementById("player-health").innerHTML =
      "<p>" + this.playerMonster.hp + "  " + this.playerMonster.fullhp + "</p>"; // appends hp to our player-health element in our table
    ...
    document.getElementById("opponent-health").innerHTML =
      "<p>" +
      this.opponentMonster.hp +
      " " +
      this.opponentMonster.fullhp +
      "</p>";
    ...
    for (let i = 0; i < 4; i++) {
      // now we add event listener so buttons work
      ...
      function addHandler(btn, move, mon1, mon2) {
        ...
        btn.addEventListener("click", (e) => {
          ...
          setTimeout(
            ...
            "player-health",
            "Opponent "
          ); // opponent will attack 2 seconds after we make select a move
        });
      }
  ...
  gameOver(hp) {
    // checks if there is a winner ie a pokemon has fainted, hp is the id of hp of the element that contains the pokemon's health
    let fainted =
      this.playerMonster.hp <= 0
        ? this.playerMonster
        : this.opponentMonster.hp <= 0
        ? this.opponentMonster
        : false; // if playerMonster health is less than 0, f = playerMonster, else if opponentMonster;s health is less than 0, f = false

    // let gamePrompt = that.attack.bind(that);

    if (fainted != false) {
      // if condition is true, their pokemon has fainted and they lose! so we alert them that the game is over
